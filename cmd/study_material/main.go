...
func main() {
	...
	userGRPCConn, err := grpc.NewClient(
		fmt.Sprintf("%s:%d", cfg.Services.User.Host, cfg.Services.User.Port),
	...
	repo := study_material_repo.NewStudyMaterialRepository(db, log)
	service := study_material_service.NewStudyMaterialService(repo, userClient, fileClient, log)
