...
OUTPUT_DATASET_FILE = 'mixed_dataset_modif_2.csv'
...
# Нейтральный микс с возможными спецсимволами и искажениями
def create_neutral_sample(normal_words):
    ...
    for _ in range(sample_len):
        ...
        # С шансом 20% — слегка исказим его (аналогично плохим словам)
        if random.random() < 0.2:
            word = distort_normal_word(word)

        # С шансом 10% — вставим псевдоаббревиатуру
        elif random.random() < 0.1:
            word = create_abbreviation_like()

        sample.append(word)
    return " ".join(sample)


# Лёгкое искажение обычного слова
def distort_normal_word(word):
    w = list(word)
    if random.random() < 0.3:
        for i in range(len(w)):
            if w[i].lower() in REPLACE_MAP and random.random() < 0.3:
                w[i] = REPLACE_MAP[w[i].lower()]
    if len(w) > 2 and random.random() < 0.5:
        i = random.randint(0, len(w)-2)
        w[i], w[i+1] = w[i+1], w[i]  # перестановка
    if len(w) > 1 and random.random() < 0.3:
        insert_pos = random.randint(1, len(w) - 1)
        w.insert(insert_pos, random.choice(['*', '_', '-', '~']))
    return ''.join(w)


# Генерация фейковых слов-аббревиатур
def create_abbreviation_like():
    length = random.randint(2, 7)
    chars = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyzЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮЁйцукенгшщзхъфывапролджэячсмитьбюё'
    word = ''.join(random.choice(chars) for _ in range(length))
    inserts = ['*', '-', '_', '~']
    pos = random.randint(1, len(word)-1)
    if random.random() < 0.3:
        result = word[:pos] + random.choice(inserts) + word[pos:]
    else:
        result = word[:pos] + word[pos:]
        # print(result)
    return result
